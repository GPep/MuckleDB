...
IF OBJECT_ID('sp_RandomTransactionSavings') IS NULL
  EXEC ('CREATE PROCEDURE sp_RandomTransactionSavings AS RETURN 0;')
GO

ALTER PROCEDURE [dbo].[sp_RandomTransactionSavings] @count int
AS
BEGIN
SET NOCOUNT ON;
-- =============================================
...
-- Version: 1.0
-- Description:	This Stored Procedure will create new random online transactions for savings accounts
...
SET @Lower = (SELECT MIN(account_id) FROM dbo.Account WHERE PRODUCT_ID IN (2)) ---- The lowest random number
SET @Upper = (SELECT MAX(Account_id) FROM dbo.Account WHERE PRODUCT_ID IN (2)) ---- The highest random number
...
-- EXECUTE sp_RandomTransactionSavings @count = 100